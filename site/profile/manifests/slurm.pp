...
class profile::slurm::controller (
  ...
  Optional[String] $tfe_api_url = undef,
) {
  ...
  uv::venv { 'autoscale_slurm_env':
    ...
    python       => '3.13',
    requirements => 'git+https://github.com/etiennedub/slurm-autoscale-tfe.git@mchub-proxy',
    require      => [
      Package['slurm'],
    ],
  }

  $tfe_api_url_export = $tfe_api_url ? {
    undef   => '',
    default => "export TFE_API_URL=${tfe_api_url}\n",
  }
...
EOT
  ...
  file { '/etc/slurm/env.secrets':
    ...
    owner   => 'slurm',
    mode    => '0600',
    content => "${env_secrets_base}${tfe_api_url_export}",
  }
